...
对每道错题输出完整运行轨迹：
1. 检索阶段：轮次、重写策略、质量评分、候选chunk列表、被丢弃chunk
2. 本地推理：每选项verdict/confidence/reasoning
3. LLM答题：格式化证据预览、LLM answer+reasoning、重试次数
4. self-check：是否触发、前后答案对比、verify reasoning
5. 与gold答案对比

输出到 .tmp_diagnostic_trace.txt（UTF-8）
"""
...
OUTPUT_FILE = PROJECT_ROOT / ".tmp_diagnostic_trace.txt"

ERROR_QIDS = [
    "fin_a_004", "fin_a_008", "fin_a_009", "fin_a_011", "fin_a_014",
    "fin_a_016", "fin_a_018", "fin_a_019", "fin_a_020",
]

GOLD_ANSWERS = {
    "fin_a_004": "ABD", "fin_a_008": "B", "fin_a_009": "ABC",
    "fin_a_011": "ACD", "fin_a_014": "AB", "fin_a_016": "AD",
    "fin_a_018": "A", "fin_a_019": "AB", "fin_a_020": "AB",
}
...
def answer_with_qwen_traced(
    ...
    """带追踪的 answer_with_qwen，记录重试次数和每次尝试的详情。"""
    trace: dict = {
        "attempts": [],
...
def main() -> None:
    ...
    for raw in all_questions:
        ...
        out.append(f"# 问题: {raw['question'][:150]}")
        for k in OPTION_KEYS:
            if k in options_dict:
                out.append(f"#   选项{k}: {options_dict[k][:100]}")
        out.append(f"{'#' * 100}")
        ...
        if qwen_result:
            ...
            out.append(f"  model: {qwen_result.model}")
            out.append(f"\n  --- LLM推理文本 ---")
            out.append(f"  {qwen_result.reasoning}")
        else:
            out.append(f"  ⚠️ LLM返回None，使用默认答案A")
        ...
        if do_self_check and qwen_result:
            try:
                ...
                if verified:
                    ...
                    out.append(f"  verify_tokens: {verified.total_tokens}")
